...
const options = {
  ...
  secret: {
    name: 'secret',
    usage: 'A secret in KEY=VALUE form',
  },
  meta: {
    name: 'meta',
  ...
  secretsFile: {
    name: 'secrets-file',
    usage: 'Path to a file with secrets',
    alternate: 'secretsFile',
  },
  path: {
    name: 'path',
...
const optionsService = {
  ...
  getSecretsOption() {
    return this.getKeyValueOptionByName(options.secret.name);
  },
  ...
  getSecretsFileOption(functionName) {
    let secretsFile = this.options[options.secretsFile.name];
    if (!secretsFile) {
      secretsFile = this.options[options.secretsFile.alternate];
    }
    if (secretsFile) {
      return BbPromise.resolve(secretsFile);
    }

    if (functionName) {
      return this.getFunctionObject(functionName).then(functionObject => {
        return functionObject['secrets-file'];
      });
    }
  ...
  getDeployOptions(functionName) {
    return this.getSecretsFileOption(functionName).then(secretsFile => {
      return this.getMetaOption().then(meta => {
        return this.getSecretsOption().then(secrets => {
          return this.getScheduleOption(functionName).then(schedule => {
            return { meta, secrets, secretsFile, schedule };
          });
        });
      })
    });
